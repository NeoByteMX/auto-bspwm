...
if [ "$user" == "root" ]; then
	...
	sleep 2
	sudo apt install -y kitty rofi feh xclip ranger i3lock-fancy scrot scrub wmname imagemagick cmatrix btop fastfetch python3-pip procps tty-clock fzf lsd bat pamixer flameshot
	if [ $? != 0 ] && [ $? != 130 ]; then
